...
const PersonalInfoForm = ({data, onChange, removeBackground,setRemoveBackground}) => {
  ...
  const fields = [
    ...
    {key:"linkdin", label:"LinkedIn Profile", icon : Linkedin, type:"url"},
  ...
  return (
    <div>
      ...
      <div className='flex items-center gap-2'>
        <label>
          {data.image ? (
            <img src={typeof data.image == 'string' ? data.image : URL.createObjectURL(data.image)} alt="User Image" className='w-16 h-16 rounded-full object-cover mt5 ring ring-slate-300 hover:opacity-80' />
          ) : (
            <div className='inline-flex items-center gap-2 mt-5 text-slate-600 hover:text-shadow-slate-700 cursor-pointer'>
